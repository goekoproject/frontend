...
export class DemoResultComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.smeRecomendationBody = new SmeRecomendationParams(
      this._demoService.getDataForm()
    );
  ...
  private _getSmeRecomendations() {
    this._smeService
      .getRecommendations(this.smeRecomendationBody)
      .subscribe((sme) => {
        if (sme && Array.isArray(sme.recommendations)) {
          const smeRecomendation = this._filterSmeRecomendations(
            sme.recommendations
          );
