...
const mapToCategoriesSectionForOrderBy = (classificationCategory: ClassificationCategory) => {
...
export const compareWithClassificationCategory = (c1: ClassificationCategory, c2: ClassificationCategory) => c1.code === c2.code
@Injectable()
export class SmeAnalysisService {
  ...
  dataCategorySelected = signal<ClassificationCategory>(NULL_CLASSIFICATION_CATEGORY)
  ...
  private _getClassificationForCategory$(codeCategory: string): Observable<ClassificationCategory> {
    return this.classificationCategoryService
      ...
      .pipe(map((classificationCategory) => mapToCategoriesSectionForOrderBy(classificationCategory)))
  }
  private _getClassificationForCategoryTranslated() {
    this._getClassificationForCategory$(this.categorySelected().code).subscribe((dataCategory) => {
      if (dataCategory) {
