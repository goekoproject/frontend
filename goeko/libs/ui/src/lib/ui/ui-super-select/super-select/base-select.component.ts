...
@Directive()
export abstract class BaseSelectComponent implements OnInit, ControlValueAccessor, AfterContentInit {
	...
	@ContentChildren(SuperOptionComponent, { descendants: true })
	optionElement!: QueryList<SuperOptionComponent & Highlightable>;

	@Output() valueChange: EventEmitter<{ value: string; isUserInput: boolean }> = new EventEmitter();
	...
	updateLabelMulti(): void {
		this.labelMulti = this.selectedMulti.length ? this.selectedMulti.map((s) => s.titleMulti).join() : this.label;
