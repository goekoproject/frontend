import { AfterContentInit, ChangeDetectorRef, Directive, ElementRef, Host, HostListener, Renderer2 } from '@angular/core'
...
export class SelectSubcategoryProductDirective implements AfterContentInit {
  get wrappersSubCategory(): Array<any> {
    return this._elementRef?.nativeElement?.parentElement?.parentElement.querySelectorAll('goeko-select-subcategory-product')
  }
  ...
  private _onFocusElement() {
    ...
    this._renderer.listen(this.inputElement, 'focus', (inputElement: FocusEvent) => {
      if ((inputElement?.currentTarget as HTMLInputElement)?.type === TYPE_FIELD.RADIO) {
        this._showSubCategorySelected(inputElement.currentTarget as any)
      }
    })
  }
