...
export class LoadingService {
	...
	start(loadingEvent: string) {
		...
		if (loadingEvent.includes('discrepancies')) {
			return;
		}
	...
	stop(loadingEvent: string) {
		setTimeout(() => {
			this._loadingEvents = this._loadingEvents.filter((loadingEv) => loadingEv !== loadingEvent);
			if (this._loadingEvents.length === 0 && this._loading$.getValue() === true) {
