...
export class SmeAnalysisStoreService {
  ...
  getCurrentAnalysis() {
    if (!this._currentAnalysis.value) {
      const currentAnalysis =
        this._sessionStoregeService.getItem<any>(SS_SME_FORM);
      this._currentAnalysis.next(currentAnalysis);
  ...
  getDetailEcosolutions() {
    if (!this._detailEcosolutions.value) {
      const detailEcosolutions = this._sessionStoregeService.getItem<any>(
        SS_ECOSOLUTION_DETAIL
      );
      this._detailEcosolutions.next(detailEcosolutions);
